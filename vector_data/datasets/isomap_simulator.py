...
class IsomapSimulator(BaseSimulator):
    def __init__(self, latent_dim=3, data_dim=4096, kappa=6.0, epsilon=0.,latent_distribution='mix_of_vonMises',noise_type='gaussian'):
        super().__init__()
        ...
        self.data = np.load(r'/storage/homefs/ch19g182/Python/inflation_deflation_estimate_d/main/datasets/face_data.npy')
        self.n_data = 698
    ...
    def sample_and_noise(self,n,parameters=None, sig2 = 0.0, mode='numpy'):
        # theta, phi = self._draw_z(n)
        idx = np.random.choice(range(0,self.n_data),n, replace=False)
        x =self.data[idx,:]
        # self._transform_z_to_x(theta,phi,mode='numpy')
