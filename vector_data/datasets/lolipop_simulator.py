...
class LolipopSimulator(BaseSimulator):
    ...
    def sample(self, n, parameters=None,mode='numpy'):
        ...
        x_stick = np.stack([ alpha, alpha, np.zeros(n_stick)], axis=1) 
    ...
    def _transform_z_to_disk(self, phi, r, mode='train'):
        ...
        d1x = r *  np.cos(phi)
        d1y = r *  np.sin(phi)
        x = np.stack([ d1x + 2, d1y + 2, np.zeros(len(phi))], axis=1)  #shift disk
