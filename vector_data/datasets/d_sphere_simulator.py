...
class d_SphereSimulator(BaseSimulator):
    ...
    def sample(self, n, parameters=None,mode='numpy'):
        x_ = np.random.randn(n,int(self._latent_dim) )  #self._transform_z_to_x(theta,phi,mode='numpy')
        x = normalize(x_,axis=1)
        
        for i in range( int(self._data_dim - self._latent_dim) ):
            dz =   np.zeros([n,1])
    ...
    def sample_and_noise(self,n,parameters=None, sig2 = 0.0, mode='numpy'):
        ...
        noise = self.create_noise(x,sig2)
    ...
    def create_noise(self,x,sig2):
        if self._noise_type == 'gaussian':
            ...
            noise = np.sqrt(sig2) * np.random.randn(x.shape[0],1) * x
        elif self._noise_type == 'uniform':
            noise = np.sqrt(sig2 + self._epsilon**2) * (np.random.rand(*x.shape)-0.5)   #isotropic uniform!
