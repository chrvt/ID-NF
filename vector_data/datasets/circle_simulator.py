...
class CircleSimulator(BaseSimulator):
    def __init__(self, latent_dim=1, data_dim=2, kappa=6.0, epsilon=0.,latent_distribution='mix_of_vonMises',noise_type='gaussian'):
        super().__init__()
    ...
    def sample(self, n, parameters=None,mode='numpy'):
        phi = self._draw_z(n)
        x = self._transform_z_to_x(phi,mode='numpy')
        return x + self._epsilon * np.random.randn(*x.shape)
    ...
    def sample_and_noise(self,n,parameters=None, sig2 = 0.0, mode='numpy'):
        phi = self._draw_z(n)
        x = self._transform_z_to_x(phi,mode='numpy')
        noise = self.create_noise(x,phi,sig2)
        return np.stack([x, noise],axis=-1)   

    def create_noise(self,x,phi,sig2):
    ...
    def _draw_z(self, n):
        if self._latent_distribution == 'unimodal':
            phi_ = np.random.vonmises(self.mu,self.kappa,n) 
        elif self._latent_distribution == 'uniform':
            phi_ = np.random.random(n) * 2 * np.pi
        return phi_
    
    
    def _transform_z_to_x(self,phi, mode='train'):
        n_samples = len(phi)
        d1x = 3*np.cos(phi)
        d1y = 3*np.sin(phi)
        x = np.stack([ d1x, d1y], axis=1) 
        for i in range(self._data_dim-2):
            dz =   np.zeros([n_samples,1])
