...
class SwissRollSimulator(BaseSimulator):
    ...
    def calculate_sigma_bound(self,u_, v_): #theta=v, phi = u
        ...
        def unimodal(u,v,mu,m):
            kappa = 2
            return np.exp(kappa*np.cos( self._translate(u) - self._translate(mu) ) +kappa*np.cos( self._translate(v) - self._translate(m) )) / i0(kappa)**2
       
        if self._latent_distribution == 'mixture':
            kappa, mu11, mu12, mu21, mu22, mu31, mu32 = 2, 0.1, 0.1, 0.5, 0.8, 0.8, 0.8 
            bound =  (2 * self._density([u_,v_]) ) / ( unimodal(u_,v_,mu11,mu12) * ( circle_term(-u_,0,alpha=mu11,kappa=kappa) * 21**2 + circle_term(0,v_,alpha=mu12,kappa=kappa) * (3*np.pi)**2 * (1+(0.5+2*v_)**2)  )  
                                                  +unimodal(u_,v_,mu11,mu12) * ( circle_term(-u_,0,alpha=mu21,kappa=kappa) * 21**2 + circle_term(0,v_,alpha=mu22,kappa=kappa) * (3*np.pi)**2 * (1+(0.5+2*v_)**2)  )
                                                  +unimodal(u_,v_,mu11,mu12) * ( circle_term(-u_,0,alpha=mu31,kappa=kappa) * 21**2 + circle_term(0,v_,alpha=mu32,kappa=kappa) * (3*np.pi)**2 * (1+(0.5+2*v_)**2)  )
                                                      )
        
        elif self._latent_distribution == 'correlated':
            bound_ = 0.5 * ( circle_term(u_,v_,kappa=5) * 21**2 + circle_term(u_,v_,sign=1,kappa=5) * (3*np.pi)**2 * (1+(0.5+2*v_)**2)  )
            bound = 1/bound_
